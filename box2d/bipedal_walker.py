...
class BipedalWalker(gym.Env):
    ...
    def _reset(self):
        ...
        self.legs = []
        self.joints = []
        for i in [-1,+1]: #i=-1 => darker color => behind leg => actions[0]
    ...
    def _step(self, action):
        ...
        state = [
            ...
            0.3*vel.y*(VIEWPORT_H/SCALE)/FPS,
            self.joints[0].angle,   # This will give 1.1 on high up, but it's still OK (and there should be spikes on hiting the ground, that's normal too)
            np.sign(self.joints[0].speed / SPEED_HIP),#self.joints[0].speed / SPEED_HIP,
            self.joints[1].angle + 1.0,
            np.sign(self.joints[1].speed / SPEED_KNEE),#self.joints[1].speed / SPEED_KNEE,
            0,#1.0 if self.legs[1].ground_contact else 0.0,
            self.joints[2].angle,
            np.sign(self.joints[2].speed / SPEED_HIP),#self.joints[2].speed / SPEED_HIP,
            self.joints[3].angle + 1.0,
            np.sign(self.joints[3].speed / SPEED_KNEE),#self.joints[3].speed / SPEED_KNEE,
