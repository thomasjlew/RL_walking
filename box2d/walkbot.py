...
#    (+34,-8), (-30,-8)
#    ]
#HULL_POLY =[ #NOTE: Density of HULL_POLY is include for weight!
#    (-20,+10), (+20,+10), (+40,+0),
#    (+20,-10), (-20,-10)
#    ]
HULL_POLY =[ #NOTE: Density of HULL_POLY is include for weight!
...
class WalkBot(gym.Env):
    ...
    def _step(self, action):
        ...
        #self.joints[0].motorSpeed = 0.
        #self.joints[1].motorSpeed = 0.
        #self.joints[2].motorSpeed = 0.
        #self.joints[3].motorSpeed = 0.

        self.world.Step(1.0/FPS, 6*30, 2*30)    #world is box2d
        ...
        # To train for a real-world robot with limited information, we removed a lot of variables
        """state = [
        ...
        state = [
            ...
            0.3*vel.y*(VIEWPORT_H/SCALE)/FPS,
            self.joints[0].angle,   # This will give 1.1 on high up, but it's still OK (and there should be spikes on hiting the ground, that's normal too)
            np.sign(self.joints[0].speed / SPEED_HIP),#
            #self.joints[0].speed / SPEED_HIP,
            self.joints[1].angle + 1.0,
            np.sign(self.joints[1].speed / SPEED_KNEE),
            #self.joints[1].speed / SPEED_KNEE,
            1.0 if self.legs[1].ground_contact else 0.0,
            self.joints[2].angle,
            np.sign(self.joints[2].speed / SPEED_HIP),#
            #self.joints[2].speed / SPEED_HIP,
            self.joints[3].angle + 1.0,
            np.sign(self.joints[3].speed / SPEED_KNEE),#
            #self.joints[3].speed / SPEED_KNEE,
            1.0 if self.legs[3].ground_contact else 0.0,
            ]
        #state += [0 for l in self.lidar]#l.fraction for l in self.lidar]
